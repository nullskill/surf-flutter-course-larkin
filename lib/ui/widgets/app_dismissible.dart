...
typedef AppDismissDirectionCallback = void Function(
    AppDismissDirection direction);
...
///
/// Dragging or flinging this widget in the [AppDismissDirection] causes the child
...
class AppDismissible extends StatefulWidget {
  ...
  const AppDismissible({
    ...
    this.onResize,
    this.onDismissed,
    this.direction = AppDismissDirection.horizontal,
    this.resizeDuration = const Duration(milliseconds: 300),
    this.dismissThresholds = const <AppDismissDirection, double>{},
  ...
  final AppDismissDirection direction;
  ...
  /// [AppDismissDirection] even if it would be allowed by the [direction]
  /// property.
  ///
  /// See also:
  ///
  ...
  ///    be dismissed.
  final Map<AppDismissDirection, double> dismissThresholds;
...
class _AppDismissibleState extends State<AppDismissible>
  ...
  bool get _directionIsXAxis {
    return widget.direction == AppDismissDirection.horizontal ||
        widget.direction == AppDismissDirection.endToStart ||
        widget.direction == AppDismissDirection.startToEnd;
  }

  AppDismissDirection _extentToDirection(double extent) {
    if (extent == 0.0) return null;
    if (_directionIsXAxis) {
      switch (Directionality.of(context)) {
        case TextDirection.rtl:
          return extent < 0
              ? AppDismissDirection.startToEnd
              : AppDismissDirection.endToStart;
    ...
    return extent > 0 ? AppDismissDirection.down : AppDismissDirection.up;
  }

  AppDismissDirection get _AppDismissDirection =>
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    switch (widget.direction) {
      case AppDismissDirection.horizontal:
      case AppDismissDirection.vertical:
        _dragExtent += delta;
        break;

      case AppDismissDirection.up:
        if (_dragExtent + delta < 0) _dragExtent += delta;
        break;

      case AppDismissDirection.down:
        if (_dragExtent + delta > 0) _dragExtent += delta;
        break;

      case AppDismissDirection.endToStart:
        switch (Directionality.of(context)) {
          case TextDirection.rtl:
            if (_dragExtent + delta > 0) _dragExtent += delta;
            break;
      ...
      case AppDismissDirection.startToEnd:
        switch (Directionality.of(context)) {
          case TextDirection.rtl:
            if (_dragExtent + delta < 0) _dragExtent += delta;
            break;
  ...
  _FlingGestureKind _describeFlingGesture(Velocity velocity) {
    ...
    AppDismissDirection flingDirection;
    ...
    assert(_AppDismissDirection != null);
    if (flingDirection == _AppDismissDirection)
  ...
  Future<void> _handleDragEnd(DragEndDetails details) async {
    ...
    switch (_describeFlingGesture(details.velocity)) {
      case _FlingGestureKind.forward:
        ...
        if ((widget.dismissThresholds[_AppDismissDirection] ??
                _kDismissThreshold) >=
            1.0) {
      ...
      case _FlingGestureKind.none:
        if (!_moveController.isDismissed) {
          ...
          if (_moveController.value >
              (widget.dismissThresholds[_AppDismissDirection] ??
                  _kDismissThreshold)) {
  ...
  Future<bool> _confirmStartResizeAnimation() async {
    if (widget.confirmDismiss != null) {
      final AppDismissDirection direction = _AppDismissDirection;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.secondaryBackground != null) {
      final AppDismissDirection direction = _AppDismissDirection;
      if (direction == AppDismissDirection.endToStart ||
          direction == AppDismissDirection.up)
